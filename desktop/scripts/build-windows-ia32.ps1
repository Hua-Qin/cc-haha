...
# Build script for Windows x86 (ia32 / 32-bit) architecture.
# Environment:
...
$canonicalOutputDir = Join-Path $desktopDir 'build-artifacts\windows-ia32'
$electronOutputDir = Join-Path $desktopDir 'build-artifacts\electron'

function Write-Step {
  param([string]$Message)
  Write-Host "[build-windows-ia32] $Message"
}

function Assert-WindowsHost {
  if ($env:OS -ne 'Windows_NT') {
    throw '[build-windows-ia32] This script must run on Windows.'
  }
}
...
function Assert-Command {
  ...
  if (-not (Get-Command $Name -ErrorAction SilentlyContinue)) {
    throw "[build-windows-ia32] Missing required command: $Name"
  }
}
...
function Import-VsDevEnvironment {
  ...
  if (-not (Test-Path $vswhere)) {
    throw '[build-windows-ia32] Could not find vswhere.exe. Install Visual Studio 2022 Build Tools with the C++ workload.'
  }
  ...
  if (-not $installationPath) {
    throw '[build-windows-ia32] Missing Visual C++ build tools. Install the Desktop development with C++ workload first.'
  }

  $vsDevCmd = Join-Path $installationPath 'Common7\Tools\VsDevCmd.bat'
  if (-not (Test-Path $vsDevCmd)) {
    throw "[build-windows-ia32] Could not find VsDevCmd.bat under $installationPath"
  }

  Write-Step "Importing MSVC environment from $vsDevCmd (x86 target)"
  $env:VSCMD_SKIP_SENDTELEMETRY = '1'
  $envDump = & cmd.exe /d /s /c "`"$vsDevCmd`" -arch=x86 -host_arch=x64 >nul && set"
  if ($LASTEXITCODE -ne 0) {
    throw "[build-windows-ia32] Failed to initialize Visual Studio build environment (exit $LASTEXITCODE)"
  }
...
if ($env:SKIP_INSTALL -ne '1') {
  ...
  try {
    & bun install
    if ($LASTEXITCODE -ne 0) {
      throw "[build-windows-ia32] bun install failed in repo root (exit $LASTEXITCODE)"
    }
  } finally {
    Pop-Location
  }
  ...
  try {
    & bun install
    if ($LASTEXITCODE -ne 0) {
      throw "[build-windows-ia32] bun install failed in desktop (exit $LASTEXITCODE)"
    }
  } finally {
    Pop-Location
  }
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "[build-windows-ia32] build:sidecars failed (exit $LASTEXITCODE)"
  }

  Write-Step 'Building renderer and Electron main/preload bundles...'
  & bun run build
  if ($LASTEXITCODE -ne 0) {
    throw "[build-windows-ia32] renderer build failed (exit $LASTEXITCODE)"
  }
  & bun run build:electron
  if ($LASTEXITCODE -ne 0) {
    throw "[build-windows-ia32] Electron build failed (exit $LASTEXITCODE)"
  }

  if ($env:REBUILD_NATIVE -eq '1') {
    Write-Step 'Rebuilding native dependencies for Electron ia32 ABI...'
    & bunx electron-builder install-app-deps --arch ia32
    if ($LASTEXITCODE -ne 0) {
      throw "[build-windows-ia32] electron-builder install-app-deps failed (exit $LASTEXITCODE)"
    }
    & bun run prepare:node-pty
    if ($LASTEXITCODE -ne 0) {
      throw "[build-windows-ia32] prepare:node-pty failed (exit $LASTEXITCODE)"
    }
  }

  $args = @('electron-builder', '--win', 'nsis', '--ia32', '--publish', 'never')
  $remainingArgs = @($BuilderArgs)
  if ($remainingArgs.Count -gt 0) {
    $args += $remainingArgs
  }

  Write-Step 'Packaging Electron app (ia32)...'
  & bunx @args
  if ($LASTEXITCODE -ne 0) {
    throw "[build-windows-ia32] electron-builder failed (exit $LASTEXITCODE)"
  }
} finally {
  Pop-Location
}
...
$winUnpackedDir = Join-Path $electronOutputDir 'win-ia32-unpacked'
if (-not (Test-Path $winUnpackedDir)) {
  # electron-builder may use 'win-unpacked' even for ia32
...
if ($env:SKIP_PACKAGE_SMOKE -eq '1') {
  ...
  Write-Step 'Running package-smoke against canonical Windows ia32 artifacts...'
  Push-Location $repoRoot
  try {
    & bun run test:package-smoke --platform windows --package-kind release --artifacts-dir desktop/build-artifacts/windows-ia32
    if ($LASTEXITCODE -ne 0) {
      throw "[build-windows-ia32] package-smoke failed (exit $LASTEXITCODE)"
    }
  } finally {
    Pop-Location
  }
